...
overview = Overview()
overview.set_filter(
    filters_dict={'Country': 'Canada', 'Relative Volume': 'Over 1.5'})


market_open = datetime.strptime('08:00', '%H:%M').time()
market_close = datetime.strptime('13:43', '%H:%M').time()
timing_check = 1
...
while timing_check:
    ...
    if datetime.now().time() > market_open and datetime.now().time() < market_close and capital == 10000:
        ...
        filtered_df = df[df['Change'] > 0].sort_values(
            by=['Market Cap'], ascending=False, ignore_index=True)
...
daily_gain_data.loc["strat_2a_finviz_rel_vol_market_cap",date.today().strftime("%d/%m/%Y")] = gain_loss
...
if date.today().strftime("%d/%m/%Y") in cum_capital:
    cum_capital.loc["strat_2a_finviz_rel_vol_market_cap",date.today().strftime("%d/%m/%Y")] = cum_capital.iloc[3,-2]*(1+gain_loss/100)
else:
    cum_capital.loc["strat_2a_finviz_rel_vol_market_cap",date.today().strftime("%d/%m/%Y")] = cum_capital.iloc[3,-1]*(1+gain_loss/100)

cum_capital.to_csv('A://Projects/Ultimate Trader/cumulative_capital.csv')

print("executed strat_2a_finviz_rel_vol_market_cap")