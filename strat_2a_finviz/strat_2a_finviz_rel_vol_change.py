...
market_close = datetime.strptime('13:40', '%H:%M').time()
timing_check = 1
...
daily_gain_data.loc["strat_2a_finviz_rel_vol_change",date.today().strftime("%d/%m/%Y")] = gain_loss
...
if date.today().strftime("%d/%m/%Y") in cum_capital:
    cum_capital.loc["strat_2a_finviz_rel_vol_change",date.today().strftime("%d/%m/%Y")] = cum_capital.iloc[2,-2]*(1+gain_loss/100)
else:
    cum_capital.loc["strat_2a_finviz_rel_vol_change",date.today().strftime("%d/%m/%Y")] = cum_capital.iloc[2,-1]*(1+gain_loss/100)

cum_capital.to_csv('A://Projects/Ultimate Trader/cumulative_capital.csv')

print("executed strat_2a_finviz_rel_vol_change")