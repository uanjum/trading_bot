...
while timing_check:
    ...
    if datetime.now().time() > market_open and datetime.now().time() < market_close and capital == 10000:
        sentiment_vader.check_tsx_sentiment(pd.read_csv('A://Projects/Ultimate Trader/strat_1a_twitter_sentiment_vader/tsx_index.csv'), datetime.now().strftime('%Y-%m-%d'))
        # above function outputs a csv file with ticker sentiment called 'ticker_w_sentiment.csv'
        ticker_w_sentiment = pd.read_csv('A://Projects/Ultimate Trader/strat_1a_twitter_sentiment_vader/ticker_w_sentiment.csv').sort_values(
...
daily_gain_data.loc["strat_1a_twitter_sentiment_vader",date.today().strftime("%d/%m/%Y")] = gain_loss
...
if date.today().strftime("%d/%m/%Y") in cum_capital:
    cum_capital.loc["strat_1a_twitter_sentiment_vader",date.today().strftime("%d/%m/%Y")] = cum_capital.iloc[0,-2]*(1+gain_loss/100)
else:
    cum_capital.loc["strat_1a_twitter_sentiment_vader",date.today().strftime("%d/%m/%Y")] = cum_capital.iloc[0,-1]*(1+gain_loss/100)
...
print("executed strat_1a_twitter_sentiment_vader")
time.sleep(43200)